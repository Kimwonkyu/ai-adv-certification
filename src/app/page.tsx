...
export default function Home() {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'study' | 'material'>('dashboard');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (savedSession) {
        ...
        // Don't automatically switch to study tab, let dashboard handle it via Resume
      }
    } catch (error) {
  ...
  const startChapterStudy = (selectedChapterNames: string[], shuffleOptions: boolean, count: number, type: 'all' | '객관식' | '주관식' = 'all') => {
  ...
  const startMockExam = (shuffleOptions: boolean) => {
    // 20 MCQ + 1 Subjective
    const mcqs = shuffleArray(allQuestions.filter(q => q.type === '객관식')).slice(0, 20);
    const subjective = shuffleArray(allQuestions.filter(q => q.type === '주관식')).slice(0, 1);

    let pool = [...mcqs, ...subjective];
    pool = shuffleArray(pool); // Shuffle the order of MCQs and Subjective
    ...
    setStudyPool(pool);
    setStudyMode('AI ADV. 실전 모의고사 (20+1)');
    setTimerSeconds(3600);
    setSessionStartTime(Date.now());
    setTimeLogs([]);
    initStudy(pool, 'AI ADV. 실전 모의고사 (20+1)', 3600);
  };
        ...
        {activeTab === 'dashboard' ? (
          ...
          <div className="animate-in zoom-in-95 duration-700 max-w-2xl mx-auto py-10">
            <div className="glass-card p-10 border-emerald-500/20 bg-emerald-500/5 text-center">
              ...
              <h2 className="text-4xl font-black text-white uppercase italic italic tracking-tighter mb-2">Session Complete!</h2>
              ...
              <div className="grid grid-cols-2 gap-4 mb-10">
                ...
                <div className="bg-black/40 p-6 rounded-2xl border border-white/5">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-3xl font-black text-white">{Math.round((studyPool.length / 12) * 100)}%</span>
                  </div>
                </div>
              </div>
            ...
            {showSoftStop && (
              <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
                <div className="glass-card max-w-md w-full p-8 border-rose-500/20 bg-zinc-900 shadow-2xl">
                  ...
                  <h3 className="text-2xl font-black text-white text-center mb-2 uppercase italic">10분 목표 달성!</h3>
